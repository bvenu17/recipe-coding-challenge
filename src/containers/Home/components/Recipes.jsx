...
// api
import { getRecipe } from '../../../api/pages/Search';
...
const Recipe = ({ match }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const res = await getRecipe(match.params.search);
  ...
  const handleFavourite = (meal) => {
    let favMeals = localStorage.getItem('favMeals');
    console.log(meal);
    if (favMeals === null) {
      let meals = [];
      meals.push(meal);
      localStorage.setItem('favMeals', JSON.stringify(meals));
    } else {
      let arr = JSON.parse(favMeals);
              ...
              return (
                <Grid item xs={4}>
                  ...
                  >
                    ...
                    <Typography
                      onClick={() => handleFavourite(item.idMeal)}
                      variant='body1'
